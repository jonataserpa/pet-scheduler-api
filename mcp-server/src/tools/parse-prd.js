	...
	server.addTool({
		...
		parameters: z.object({
			...
			output: z
				.string()
				.optional()
				.describe("Output path for tasks.json file (default: tasks/tasks.json)"),
			force: z.boolean().optional().describe("Allow overwriting an existing tasks.json file."),
			projectRoot: z.string().describe("The directory of the project. Must be absolute path."),
		}),
		execute: async (args, { log, session }) => {
			try {
				...
				const { projectRoot, prdPath, tasksJsonPath } = resolveProjectPaths(rootFolder, args, log);
