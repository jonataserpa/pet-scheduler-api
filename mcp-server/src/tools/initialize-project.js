import { z } from "zod";
import { createContentResponse, createErrorResponse, handleApiResult } from "./utils.js";
	...
	server.addTool({
		...
		parameters: z.object({
			projectName: z
				.string()
				.optional()
				.describe("The name for the new project. If not provided, prompt the user for it."),
			projectDescription: z
				.string()
				.optional()
				.describe("A brief description for the project. If not provided, prompt the user for it."),
			projectVersion: z
				.string()
				.optional()
				.describe(
			...
			authorName: z
				.string()
				.optional()
				.describe("The author's name. User input not needed unless user requests to override."),
			skipInstall: z
				.boolean()
				.optional()
				.default(false)
		...
		execute: async (args, context) => {
			...
			log.info(">>> Full Context Received by Tool:", JSON.stringify(context, null, 2));
			log.info(`Context received in tool function: ${context}`);
			log.info(`Session received in tool function: ${session ? session : "undefined"}`);
