...
export async function initializeProjectDirect(args, log, context = {}) {
	...
	log.info(`SESSION extracted in direct function: ${session ? "Exists" : "MISSING or Falsy"}`);
	...
	if (args.projectRoot && args.projectRoot !== "/" && args.projectRoot !== homeDir) {
		...
		if (sessionDerivedPath && sessionDerivedPath !== "/" && sessionDerivedPath !== homeDir) {
			log.info(`Using project root derived from session: ${sessionDerivedPath}`);
