	...
	if (fs.existsSync(targetPath)) {
		...
		if (filename === "package.json") {
			...
			try {
				...
				fs.writeFileSync(targetPath, JSON.stringify(existingPackageJson, null, 2));
				...
				scripts: {
					...
					...Object.fromEntries(
						Object.entries(packageJson.scripts).filter(
							([key]) => !existingPackageJson.scripts || !existingPackageJson.scripts[key],
						),
					),
				},
				dependencies: {
					...
					...Object.fromEntries(
						Object.entries(packageJson.dependencies).filter(
							([key]) =>
								!existingPackageJson.dependencies || !existingPackageJson.dependencies[key],
						),
					),
				},
			};
		...
		console.log(
			boxen(
				chalk.cyan.bold("Things you can now do:") +
					...
					"\n" +
					chalk.white("3. ") +
					chalk.yellow("Ask Cursor Agent to parse your PRD.txt and generate tasks") +
					"\n" +
					chalk.white("   └─ ") +
					...
					"\n" +
					chalk.white("5. ") +
					chalk.yellow("Ask Cursor which task is next to determine where to start") +
					"\n" +
					chalk.white("6. ") +
					chalk.yellow("Ask Cursor to expand any complex tasks that are too large or complex.") +
					"\n" +
					chalk.white("7. ") +
					chalk.yellow(
	...
	if (fs.existsSync(mcpJsonPath)) {
		...
		try {
			...
			const hasMCPString = Object.values(mcpConfig.mcpServers).some(
				(server) =>
					server.args &&
					server.args.some((arg) => typeof arg === "string" && arg.includes("task-master-mcp")),
			);
