	...
	programInstance
		...
		.action(async (file, options) => {
			...
			if (!inputFile) {
				...
				console.log(
					chalk.yellow("No PRD file specified and default PRD file not found at scripts/prd.txt."),
				);
				console.log(
					boxen(
		...
		.action(async (options) => {
			...
			try {
				...
				if (!options.yes) {
					...
					console.log();
					console.log(
						chalk.red.bold("⚠️ WARNING: This will permanently delete the following task:"),
					);
					console.log();
					...
					if (typeof taskId === "string" && taskId.includes(".")) {
						...
						console.log(
							chalk.gray(`Parent Task: ${task.parentTask.id} - ${task.parentTask.title}`),
						);
					} else {
						// It's a main task
				...
				if (typeof taskId === "string" && taskId.includes(".")) {
					// It was a subtask
					console.log(
						boxen(chalk.green(`Subtask ${taskId} has been successfully removed`), {
							padding: 1,
							borderColor: "green",
							borderStyle: "round",
						}),
