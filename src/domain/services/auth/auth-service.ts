...
export class AuthService {
	...
	public async login(loginDto: LoginDTO): Promise<LoginResultDTO> {
		try {
			...
			await this.registerSuccessLogin(user.id, user.email, loginDto.ipAddress, loginDto.userAgent);
	...
	private async registerFailedLogin(
		...
		try {
			...
			const isSuspiciousIP = await this.loginHistoryRepository.isSuspiciousIpActivity(
				ipAddress,
				60,
				10,
			); // 60 minutos, 10 tentativas
			if (isSuspiciousIP) {
