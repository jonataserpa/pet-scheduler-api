...
export class PasswordResetService {
	...
	public async connect(): Promise<void> {
		try {
			...
			throw new Error(
				`PasswordResetService: Erro ao conectar ao Redis: ${(error as Error).message}`,
			);
		}
	}
	...
	public async disconnect(): Promise<void> {
		try {
			...
			throw new Error(
				`PasswordResetService: Erro ao desconectar do Redis: ${(error as Error).message}`,
			);
		}
	}
	...
	private async invalidateToken(token: string): Promise<void> {
		try {
			...
			throw new Error(
				`Erro ao invalidar token de recuperação de senha: ${(error as Error).message}`,
			);
		}
	}
