...
export class Notification {
	...
	public static create(
		...
		if (!Object.values(NotificationType).includes(type)) {
			throw new Error(
				`Notification: tipo inválido: ${type}. Deve ser um dos valores: ${Object.values(NotificationType).join(", ")}`,
			);
		}
		...
		if (!Object.values(NotificationStatus).includes(status)) {
			throw new Error(
				`Notification: status inválido: ${status}. Deve ser um dos valores: ${Object.values(NotificationStatus).join(", ")}`,
			);
		}
	...
	public markAsSent(sentAt: Date = new Date()): void {
		if (this._status !== NotificationStatus.PENDING) {
			throw new Error(
				`Notification: não é possível marcar como enviada uma notificação com status ${this._status}`,
			);
		}
	...
	public markAsDelivered(deliveredAt: Date = new Date()): void {
		if (this._status !== NotificationStatus.SENT) {
			throw new Error(
				`Notification: não é possível marcar como entregue uma notificação com status ${this._status}`,
			);
		}
	...
	public markAsFailed(reason: string, failedAt: Date = new Date()): void {
		if (this._status !== NotificationStatus.PENDING && this._status !== NotificationStatus.SENT) {
			throw new Error(
				`Notification: não é possível marcar como falha uma notificação com status ${this._status}`,
			);
		}
	...
	public retry(): void {
		if (this._status !== NotificationStatus.FAILED) {
			throw new Error(
				`Notification: não é possível retentar uma notificação com status ${this._status}`,
			);
		}
	...
	public updateContent(content: string): void {
		if (this._status !== NotificationStatus.PENDING) {
			throw new Error(
				`Notification: não é possível atualizar o conteúdo de uma notificação com status ${this._status}`,
			);
		}
