...
export class ScheduledService {
	...
	constructor(id: string, serviceId: string, serviceName: string, price: number, duration: number) {
		this._id = id;
...
export class Scheduling {
	...
	public static create(
		...
		if (!Object.values(SchedulingStatus).includes(status)) {
			throw new Error(
				`Scheduling: status inválido: ${status}. Deve ser um dos valores: ${Object.values(SchedulingStatus).join(", ")}`,
			);
		}
	...
	public updateTimeSlot(timeSlot: TimeSlot): void {
		if (
			...
			throw new Error(
				`Scheduling: não é possível alterar o período de um agendamento com status ${this._status}`,
			);
		}
	...
	public updateStatus(status: SchedulingStatus): void {
		...
		if (this._status === SchedulingStatus.COMPLETED && status !== SchedulingStatus.COMPLETED) {
			throw new Error(
				"Scheduling: não é possível alterar o status de um agendamento já completado",
			);
		}

		if (this._status === SchedulingStatus.CANCELLED && status !== SchedulingStatus.CANCELLED) {
		...
		if (
			...
			throw new Error(
				"Scheduling: não é possível alterar o status de um agendamento com no-show, exceto para reagendar",
			);
		}
	...
	public confirm(): void {
		if (this._status !== SchedulingStatus.SCHEDULED) {
			throw new Error(
				`Scheduling: não é possível confirmar um agendamento com status ${this._status}`,
			);
		}
	...
	public markAsInProgress(): void {
		if (
			...
			throw new Error(
				`Scheduling: não é possível marcar como em andamento um agendamento com status ${this._status}`,
			);
		}
	...
	public complete(): void {
		if (this._status !== SchedulingStatus.IN_PROGRESS) {
			throw new Error(
				`Scheduling: não é possível completar um agendamento com status ${this._status}`,
			);
		}
	...
	public markAsNoShow(): void {
		if (
			...
			throw new Error(
				`Scheduling: não é possível marcar como no-show um agendamento com status ${this._status}`,
			);
		}
	...
	public updateServices(services: ScheduledService[]): void {
		if (
			...
			throw new Error(
				`Scheduling: não é possível alterar os serviços de um agendamento com status ${this._status}`,
			);
		}
