...
export class Customer {
	...
	public static create(
		...
		if (cleanDocumentNumber.length !== 11 && cleanDocumentNumber.length !== 14) {
			throw new Error(
				"Customer: número de documento inválido. Deve ser CPF (11 dígitos) ou CNPJ (14 dígitos)",
			);
		}
	 ...
	 */
	public update(name: string, documentNumber: string, address: Address, contact: Contact): void {
		if (!name || name.trim() === "") {
			throw new Error("Customer: nome é obrigatório");
		}
