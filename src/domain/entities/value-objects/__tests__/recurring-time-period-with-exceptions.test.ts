...
describe("RecurringTimePeriodWithExceptions", () => {
	...
	const recurringPeriod = RecurringTimePeriod.create([mondayPeriod, wednesdayPeriod, fridayPeriod]);
	...
	describe("create", () => {
		...
		it("deve normalizar as datas de exceção para meia-noite", () => {
			...
			const periodWithExceptions = RecurringTimePeriodWithExceptions.create(recurringPeriod, [
				dateWithTime,
			]);
	...
	describe("removeException", () => {
		...
		it("deve remover uma exceção e retornar uma nova instância", () => {
			...
			expect(result.exceptions.some((d) => d.getTime() === wednesdayException.getTime())).toBe(
				false,
			);
		});
	...
	describe("equals", () => {
		const period1 = RecurringTimePeriodWithExceptions.create(recurringPeriod, [
			mondayException,
			wednesdayException,
		]);
