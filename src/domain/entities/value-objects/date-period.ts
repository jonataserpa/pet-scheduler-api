...
export class DatePeriod {
	 ...
	 */
	public static create(startDate: Date, endDate: Date): DatePeriod {
		// Validações
		...
		return new DatePeriod(new Date(startDate), new Date(endDate));
	}

	/**
	 * Cria um DatePeriod a partir de uma data inicial e uma duração em dias
	 */
	public static createFromDuration(startDate: Date, durationDays: number): DatePeriod {
		if (!(startDate instanceof Date) || isNaN(startDate.getTime())) {
			throw new Error("DatePeriod: data de início inválida");
		}
