...
export class Service {
  ...
  public static create(
    ...
    for (const size of petSizes) {
      if (!Object.values(PetSize).includes(size)) {
        throw new Error(`Service: tamanho de pet inválido: ${size}. Deve ser um dos valores: ${Object.values(PetSize).join(', ')}`);
      }
    }
  ...
  public removePetSize(size: PetSize): void {
    ...
    if (this._petSizes.length <= 1) {
      throw new Error('Service: não é possível remover o último tamanho de pet. Um serviço deve ter pelo menos um tamanho compatível.');
    }
