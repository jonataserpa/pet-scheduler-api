...
export class CreateCustomerUseCase {
  ...
  async execute(request: CreateCustomerRequest): Promise<Result<CreateCustomerResponse>> {
    try {
      ...
      const existingCustomer = await this.customerRepository.findByDocumentNumber(request.documentNumber);
      if (existingCustomer) {
      ...
      if (request.sendWelcomeNotification !== false) {
        try {
          ...
          logger.info('Notificação de boas-vindas enviada com sucesso', { customerId: customer.id });
        } catch (error) {
