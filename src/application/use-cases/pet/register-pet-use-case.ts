...
export class RegisterPetUseCase {
  ...
  async execute(request: RegisterPetRequest): Promise<Result<RegisterPetResponse>> {
    try {
      ...
      if (!customer) {
        logger.warn('Cliente não encontrado para registro de pet', { customerId: request.customerId });
        return Result.fail({
      ...
      if (request.size === PetSize.LARGE) {
        ...
        logger.info('Pet de porte grande registrado - alertando cliente sobre restrições potenciais', {
          customerId: request.customerId,
