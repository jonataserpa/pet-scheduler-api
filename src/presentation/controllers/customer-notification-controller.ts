	...
	async sendWelcomeNotification(req: Request, res: Response): Promise<void> {
		try {
			...
			res.status(500).json({
				status: "error",
				message:
					error instanceof Error ? error.message : "Erro ao enviar notificação de boas-vindas",
			});
		}
	}
	...
	async sendPendingCheckupReminders(req: Request, res: Response): Promise<void> {
		try {
			...
			res.status(500).json({
				status: "error",
				message:
					error instanceof Error ? error.message : "Erro ao enviar lembretes de checkup pendentes",
			});
		}
	}
}
