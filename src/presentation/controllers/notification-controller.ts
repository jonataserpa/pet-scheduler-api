...
export class NotificationController {
	...
	async getNotificationsByScheduling(req: Request, res: Response): Promise<void> {
		try {
			...
			const notifications =
				await this.notificationService.getNotificationsForScheduling(schedulingId);
	...
	async processPendingNotifications(req: Request, res: Response): Promise<void> {
		try {
			...
			res.status(500).json({
				status: "error",
				message:
					error instanceof Error ? error.message : "Erro ao processar notificações pendentes",
			});
		}
	}
}
