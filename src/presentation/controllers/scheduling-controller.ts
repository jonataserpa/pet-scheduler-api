...
export class SchedulingController {
	...
	async createScheduling(req: Request, res: Response): Promise<void> {
		try {
			...
			if (
				...
				res.status(400).json({
					error:
						"Dados incompletos. Data/hora de início e fim, cliente, pet e serviços são obrigatórios",
				});
				return;
			}
			...
			if (start >= end) {
				res
					.status(400)
					.json({ error: "A data/hora de início deve ser anterior à data/hora de fim" });
				return;
			}
			...
			logger.debug("Preço total calculado para o agendamento:", {
				totalPrice,
				serviceCount: scheduledServices.length,
			});
	...
	async updateStatus(req: Request, res: Response): Promise<void> {
		try {
			...
			// Atualizar o status
			const updatedScheduling = await this.schedulingRepository.updateStatus(
				id,
				status as SchedulingStatus,
			);
			...
			logger.error("Erro ao atualizar status do agendamento:", {
				error,
				id: req.params.id,
				body: req.body,
			});
	...
	async addNotes(req: Request, res: Response): Promise<void> {
		try {
			...
			logger.error("Erro ao adicionar notas ao agendamento:", {
				error,
				id: req.params.id,
				body: req.body,
			});
	...
	async getSchedulingsByCustomer(req: Request, res: Response): Promise<void> {
		try {
			...
			logger.error("Erro ao buscar agendamentos do cliente:", {
				error,
	...
	async getSchedulingsByPeriod(req: Request, res: Response): Promise<void> {
		try {
			...
			if (start > end) {
				res
					.status(400)
					.json({ error: "A data de início deve ser anterior ou igual à data de fim" });
				return;
			}
