...
export class CustomerController {
	...
	async createCustomer(req: Request, res: Response): Promise<void> {
		try {
			...
			if (!name || !email || !phone || !documentNumber) {
				res.status(400).json({
					error: "Dados incompletos. Nome, email, telefone e documento são obrigatórios.",
				});
				return;
			}
	...
	async getCustomerByDocument(req: Request, res: Response): Promise<void> {
		try {
			...
			logger.error("Erro ao buscar cliente por documento:", {
				error,
	...
	async getCustomerPets(req: Request, res: Response): Promise<void> {
		try {
			...
			const pets = await this.petRepository.findByCustomerId(id, includeInactive === "true");
	...
	async checkDocumentExists(req: Request, res: Response): Promise<void> {
		try {
			...
			logger.error("Erro ao verificar documento:", {
				error,
				documentNumber: req.params.documentNumber,
			});
