...
export class PrismaLoginHistoryRepository implements LoginHistoryRepository {
   ...
   */
  private mapDomainStatusToPrisma(status: string): 'SUCCESS' | 'FAILED' | 'LOCKED' | 'PASSWORD_RESET' | 'SUSPICIOUS' {
    switch (status) {
      case 'success':
        return 'SUCCESS';
      case 'failed':
   ...
   */
  private mapPrismaStatusToDomain(status: string): 'success' | 'failed' | 'locked' | 'password_reset' | 'suspicious' {
    switch (status) {
      case 'SUCCESS':
        return 'success';
      case 'FAILED':
