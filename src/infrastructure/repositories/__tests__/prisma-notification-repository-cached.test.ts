...
describe("PrismaNotificationRepositoryCached", () => {
	...
	beforeEach(() => {
		...
		mockBaseRepository = new PrismaNotificationRepositoryRefactored(
			mockPrismaClient,
		) as jest.Mocked<PrismaNotificationRepositoryRefactored>;
		(PrismaNotificationRepositoryRefactored as jest.Mock).mockImplementation(
			() => mockBaseRepository,
		);

		repository = new PrismaNotificationRepositoryCached(mockPrismaClient, mockCache, mockMonitor);
	});
	...
	describe("findById", () => {
		it("deve retornar notificação do cache quando disponível", async () => {
			...
			expect(mockMonitor.measure).toHaveBeenCalledWith("findById", expect.any(Function), { id });
		});
	...
	describe("create", () => {
		it("deve criar notificação e atualizar cache", async () => {
			...
			expect(result).toBe(mockNotification);
			expect(mockMonitor.measure).toHaveBeenCalledWith("create", expect.any(Function), {
				id,
				type,
				schedulingId,
			});
	...
	describe("delete", () => {
		it("deve excluir a notificação e limpar o cache", async () => {
			...
			expect(mockCache.invalidateAllLists).toHaveBeenCalled();
			expect(mockMonitor.measure).toHaveBeenCalledWith("delete", expect.any(Function), { id });
		});
	});
