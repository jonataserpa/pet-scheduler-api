...
describe('PrismaNotificationRepositoryRefactored', () => {
  ...
  describe('create', () => {
    it('deve criar uma nova notificação', async () => {
      ...
      const toDomainSpy = jest.spyOn(NotificationMapper, 'toDomain')
        .mockReturnValue(Notification.create(
  ...
  describe('operações de status', () => {
    it('deve marcar uma notificação como enviada', async () => {
      ...
      const toDomainSpy = jest.spyOn(NotificationMapper, 'toDomain')
        .mockReturnValue(Notification.create(
          id, NotificationType.EMAIL, 'Teste de notificação', mockUpdatedNotification.schedulingId, 
          NotificationStatus.SENT, new Date()
        ));
    ...
    it('deve atualizar o conteúdo usando transação', async () => {
      ...
      const toDomainSpy = jest.spyOn(NotificationMapper, 'toDomain')
        .mockReturnValue(Notification.create(
          id, NotificationType.EMAIL, content, mockUpdatedNotification.schedulingId, 
          NotificationStatus.PENDING, new Date()
        ));
  ...
  describe('findAll e count', () => {
    it('deve buscar notificações com filtros', async () => {
      ...
      // Spy no NotificationMapper
      jest.spyOn(NotificationMapper, 'toDomain')
        .mockImplementation((prismaNotification) => Notification.create(
