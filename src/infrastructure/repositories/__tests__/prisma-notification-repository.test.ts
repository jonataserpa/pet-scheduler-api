...
describe('PrismaNotificationRepository', () => {
  ...
  describe('create', () => {
    ...
    it('deve lançar erro quando criar com dados inválidos', async () => {
      ...
      // Act & Assert
      await expect(repository.create(id, type, '', uuidv4())).rejects.toThrow('Conteúdo da notificação é obrigatório');
      await expect(repository.create(id, type, 'Content', '')).rejects.toThrow('ID do agendamento é obrigatório');
      await expect(repository.create('', type, 'Content', uuidv4())).rejects.toThrow('ID da notificação é obrigatório');
    });
  });
  ...
  describe('findById', () => {
    ...
    it('deve lançar erro quando ID é vazio', async () => {
      // Act & Assert
      await expect(repository.findById('')).rejects.toThrow('ID da notificação é obrigatório para busca');
    });
  });
  ...
  describe('operações de status usando transações', () => {
    ...
    it('deve marcar uma notificação como falha e validar o motivo', async () => {
      ...
      // Verificar validação
      await expect(repository.markAsFailed(id, '')).rejects.toThrow('Motivo da falha é obrigatório');
    });
  });
  ...
  describe('updateContent', () => {
    it('deve atualizar o conteúdo usando transação', async () => {
      ...
      await expect(repository.updateContent('', content)).rejects.toThrow('ID da notificação é obrigatório');
      await expect(repository.updateContent(id, '')).rejects.toThrow('Conteúdo da notificação é obrigatório');
    });
  });
  ...
  describe('delete', () => {
    ...
    it('deve lançar erro quando tentar excluir com ID vazio', async () => {
      // Act & Assert
      await expect(repository.delete('')).rejects.toThrow('ID da notificação é obrigatório para exclusão');
    });
  });
}); 