 ...
 */
export class PrismaNotificationRepositoryRefactored
	...
	async save(notification: Notification): Promise<Notification> {
		try {
			...
			if (validationResult.isFailure) {
				...
				throw new Error(
					`Erro ao validar notificação: ${(validationResult.error as DomainError).message}`,
				);
			}
		...
		try {
			...
			const notifications = await this.prisma.notification.findMany({
				where: {
					...
					status: status
						? { equals: NotificationMapper.mapDomainStatusToPrisma(status) }
						: undefined,
	...
	async findBySchedulingId(schedulingId: string): Promise<Notification[]> {
		try {
			...
			if (validationResult.isFailure) {
				logger.error("Erro na validação:", validationResult.error);
				throw new Error(
					`Erro ao validar agendamento: ${(validationResult.error as DomainError).message}`,
				);
			}
