import {
	...
	SchedulingStatus,
	ScheduledService,
} from "../../domain/entities/scheduling.js";
import { TimeSlot } from "../../domain/entities/value-objects/time-slot.js";
import {
	SchedulingFilter,
	SchedulingRepository,
} from "../../domain/repositories/scheduling-repository.js";
	...
	implements SchedulingRepository
{
	...
	async findAll(filter: SchedulingFilter, limit?: number, offset?: number): Promise<Scheduling[]> {
		try {
			...
			return schedulings.map((scheduling) =>
				this.mapToDomain(scheduling as unknown as ExtendedScheduling),
			);
		} catch (error) {
	...
	async findConflicts(timeSlot: TimeSlot, excludeId?: string): Promise<Scheduling[]> {
		try {
			...
			return this.handleError(error, "findConflicts", {
	...
	async updateTimeSlot(id: string, timeSlot: TimeSlot): Promise<Scheduling> {
		try {
			...
			return this.handleError(error, "updateTimeSlot", {
				id,
				startTime: timeSlot.startTime,
				endTime: timeSlot.endTime,
			});
	...
	private mapToDomain(prismaScheduling: ExtendedScheduling): Scheduling {
		...
		// Criamos o TimeSlot
		const timeSlot = TimeSlot.create(prismaScheduling.startTime, prismaScheduling.endTime);
